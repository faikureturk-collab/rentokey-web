...
Deneme hesabı akışı daha sonra sadeleştirilerek tek adıma indirildi ve gerçek Supabase Auth kaydına bağlandı. Web sitesi yalnız ad-soyad, iş e-postası, şifre ve sözleşme onayı alıyor. Başarılı `signUp` isteğinden sonra kullanıcıdan e-posta adresini doğrulaması isteniyor; firma adı ve filo büyüklüğü doğrulamadan sonra `app.rentokey.com` içindeki onboarding akışında tanımlanıyor. Form gönderim sırasında kilitleniyor, çift gönderimi engelliyor ve Supabase hata kodlarını Türkçe kullanıcı mesajlarına dönüştürüyor. `NEXT_PUBLIC_SUPABASE_URL` ve `NEXT_PUBLIC_SUPABASE_ANON_KEY` değerleri 24 Ağustos 2026 tarihinde kullanıcı tarafından Vercel proje ayarlarına eklendi; anahtar değerleri güvenlik amacıyla dokümana veya Git'e yazılmadı.
