    ...
    <section className="overflow-hidden bg-[radial-gradient(circle_at_78%_28%,rgba(24,184,120,0.11),transparent_36%)] py-8 sm:py-10 lg:py-8">
      <div className="mx-auto grid w-full max-w-[1320px] items-center gap-9 px-5 lg:grid-cols-[0.84fr_1.16fr] lg:gap-9 xl:gap-12">
        <div className="max-w-[590px]">
          <span className="inline-flex max-w-full items-center gap-2 rounded-full border border-brand-green/25 bg-brand-green/5 px-4 py-1.5 text-xs font-semibold leading-snug text-brand-green-dark sm:text-sm">
          ...
          <h1 className="mt-5 text-4xl font-extrabold leading-[1.04] tracking-[-0.045em] text-brand-navy sm:text-5xl xl:text-[54px]">
            <span className="block">{en ? "Run the operation. " : "Operasyonu yönetin. "}</span>
            <span className="block text-brand-green">{en ? "Not the chaos." : "Yoğunluğu değil."}</span>
          </h1>
          ...
          <div className="mt-4 flex flex-wrap gap-x-3 gap-y-1 text-[11px] font-semibold text-brand-navy/48 sm:text-xs">
            {trustItems.map((item, index) => (
              <span key={item} className={`inline-flex items-center gap-3 ${index === 2 ? "basis-full sm:basis-auto" : "whitespace-nowrap"}`}>
                {index > 0 && <i aria-hidden="true" className={index === 2 ? "hidden sm:inline" : undefined}>·</i>}
                {item}
              </span>
            ))}
          </div>
