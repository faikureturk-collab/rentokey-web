  ...
  return (
    <div aria-label={en ? "Sample RentOkey operation centre" : "Örnek RentOkey operasyon merkezi"} className="overflow-hidden rounded-2xl border border-brand-navy/10 bg-brand-navy-deep text-white shadow-xl shadow-brand-navy/10">
      ...
      <div className="px-3 pb-3">
        ...
        <div className="overflow-hidden rounded-lg border border-white/10">{rows.map(([time, title, state], index) => <div key={`${time}-${title}`} className={`grid grid-cols-[2.8rem_1fr_auto] items-center gap-2 bg-white/[0.055] px-2.5 py-2 text-[11px] ${index ? "border-t border-white/10" : ""}`}><span className="font-bold text-sky-300">{time}</span><span className="truncate text-white/75">{title}</span><span className={index === 2 ? "text-amber-300" : "text-brand-green"}>{state}</span></div>)}</div>
      </div>
      <p className="border-t border-white/10 bg-white/[0.035] px-4 py-3 text-xs leading-relaxed text-white/55">{en ? "Handovers, returns, vehicle status and outstanding balances in one working view." : "Teslim, iade, araç durumu ve açık bakiye aynı çalışma ekranında."}</p>
    </div>
  );
}
...
export function ReservationTimelinePreview({ locale }: { locale: "tr" | "en" }) {
  ...
  return (
    <div aria-label={en ? "Sample car rental reservation timeline" : "Örnek araç kiralama rezervasyon zaman çizelgesi"} className="overflow-hidden rounded-2xl border border-brand-navy/10 bg-brand-navy-deep text-white shadow-xl shadow-brand-navy/10">
      ...
      <div className="p-2">{vehicles.map((vehicle, index) => <div key={vehicle} className="grid min-h-9 grid-cols-[6.4rem_1fr] items-center border-b border-white/[0.07] last:border-0"><span className="px-1.5 text-[9px] text-white/65">{vehicle}</span><div className="relative h-6"><span className={`absolute top-1 h-4 rounded ${index === 1 ? "left-[18%] w-[62%] bg-brand-green" : index === 2 ? "left-[45%] w-[36%] bg-amber-500" : index === 3 ? "left-[4%] w-[30%] bg-[#705DE8]" : "left-[4%] w-[53%] bg-brand-blue"}`}><span className="block truncate px-1.5 pt-0.5 text-[7px] font-semibold text-white">{index === 2 ? (en ? "Maintenance" : "Bakım") : (en ? "Reservation" : "Rezervasyon")}</span></span>{index === 0 && <span className="absolute left-[42%] top-1 h-4 w-[34%] rounded border border-red-300 bg-red-500"><span className="block truncate px-1.5 pt-0.5 text-[7px] font-semibold text-white">{en ? "Conflict" : "Çakışma"}</span></span>}</div></div>)}</div>
      <div className="mx-2 mb-2 grid gap-1 rounded-lg border border-red-300/25 bg-red-400/10 px-2.5 py-2 text-[10px] sm:grid-cols-[1fr_auto] sm:items-center"><span className="font-semibold text-red-200">{en ? "ROK 102 · R-2590 conflicts" : "ROK 102 · R-2590 çakışıyor"}</span><span className="font-bold text-brand-green">{en ? "Suitable: ROK 104 · Same class" : "Uygun: ROK 104 · Aynı sınıf"}</span></div>
      <div className="flex items-center gap-3 border-t border-white/10 px-3 py-2.5 text-[8px] text-white/45"><span className="flex items-center gap-1"><i className="h-2 w-2 rounded-sm bg-brand-blue" />{en ? "Rented" : "Kirada"}</span><span className="flex items-center gap-1"><i className="h-2 w-2 rounded-sm bg-brand-green" />{en ? "Available" : "Müsait"}</span><span className="flex items-center gap-1"><i className="h-2 w-2 rounded-sm bg-red-500" />{en ? "Conflict" : "Çakışma"}</span></div>
    </div>
  );
}
